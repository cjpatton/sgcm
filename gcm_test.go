...
//
// Change log
// - Change calls cipher.NewGCM() and cipher.NewGCMWithNonceSize()
//   to NewGCM() and NewGCMWithNonceSize(() resp. (This is needed to test
//   the changes to gcm.go.)
...
func TestAESGCM(t *testing.T) {
	for i, test := range aesGCMTests {
		...
		aesgcm, err := NewGCMWithNonceSize(aes, len(nonce))
		if err != nil {
			t.Fatal(err)
		}
...
func TestGCMCounterWrap(t *testing.T) {
	...
	for i, test := range tests {
		...
		aead, err := NewGCMWithNonceSize(key, len(nonce))
		if err != nil {
			t.Fatal(err)
		}
...
func TestGCMAsm(t *testing.T) {
	...
	newAESGCM := func(key []byte) (asm, generic cipher.AEAD, err error) {
		...
		asm, err = NewGCM(block)
		if err != nil {
			return nil, nil, err
		}
		generic, err = NewGCM(wrap(block))
		if err != nil {
			return nil, nil, err
		}
...
func TestGcmAdd32(t *testing.T) {
	// Tests that gcmAdd32(&data, offset) is the same as running gcmInc32(&data)
	// offset times.
	offset := uint32(313375)
	var counterBlock1, counterBlock2 [16]byte

	for i := 0; i < int(offset); i++ {
		gcmInc32(&counterBlock1)
	}

	gcmAdd32(&counterBlock2, offset)

	eq := true
	for i := 0; i < 16; i++ {
		eq = eq && (counterBlock1[i] == counterBlock2[i])
	}
	if !eq {
		t.Errorf("gcmAdd32(&counterBlock2, %v): got %v, want %v",
			offset, counterBlock2, counterBlock1)
	}
}

var xorKeyStreamTestSlices = []struct {
	start, end int
}{
	{0, 16},
	{2, 16},
...
func TestXORKeyStream(t *testing.T) {
	test := aesGCMTests[len(aesGCMTests)-1]
	key, _ := hex.DecodeString(test.key)
	aes, err := aes.NewCipher(key)
	if err != nil {
		t.Fatal(err)
	...
	for _, testSlice := range xorKeyStreamTestSlices {
		start, end := testSlice.start, testSlice.end
		aesgcm.(RandomAccessStream).XORKeyStream(plaintext2[start:end], ct[start:end], nonce, start)
		if !bytes.Equal(plaintext2[start:end], plaintext[start:end]) {
			t.Errorf("%d to %d: got\n%x, want\n%x", start, end, plaintext2[start:end], plaintext[start:end])
		}
	}
}

var testChunkSizes = []int{
	2, 3, 11, 16, 31, 32, 49, 62, 112, 292, 293, 400, 4000,
}
...
func TestGCMEncryptor(t *testing.T) {
	for i, test := range aesGCMTests {
		...
		enc, _, err := NewStreamingGCMWithNonceSize(aes, len(nonce))
		if err != nil {
			t.Fatal(err)
		}
		...
		for _, chunk := range testChunkSizes {
			...
			if !bytes.Equal(c, result) {
				t.Errorf("#%d chunk size %d: got \n%x, want \n%x", i, chunk, c, result)
			}
		}
	}
}

func TestGCMDecryptor(t *testing.T) {
	for i, test := range aesGCMTests {
		key, _ := hex.DecodeString(test.key)
		aes, err := aes.NewCipher(key)
		if err != nil {
		...
		_, dec, err := NewStreamingGCMWithNonceSize(aes, len(nonce))
		if err != nil {
			t.Fatal(err)
		}
		...
		for _, chunk := range testChunkSizes {
			...
			if !bytes.Equal(p, plaintext) {
				t.Errorf("#%d chunk size %d: got \n%x, want \n%x", i, chunk, p, plaintext)
			}
		}
	}
}

func TestGCMDecryptorVerifyOnly(t *testing.T) {
	test := aesGCMTests[len(aesGCMTests)-1]
	key, _ := hex.DecodeString(test.key)
	aes, err := aes.NewCipher(key)
	if err != nil {
